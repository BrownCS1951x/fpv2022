...
-- (ignore this line)
local attribute [-instance] coe_sort_bool
...
/-!
We can always check if a boolean is `tt` or `ff`. 
We can't always tell if a proposition is provable or not. 
But mathematicians talk about undecidable propositions all the time!
-/
...
@[instance] def fun.inhabited {α β : Type} [inhabited β] :
  inhabited (α → β) :=
{ default := λa : α, inhabited.default }
