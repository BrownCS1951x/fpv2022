...
Not only do terms have a type, but also types have a type. For example, by the PAT
principle:
...
section 
local infixr (name := cardinal_pow) ^ := @has_pow.pow cardinal cardinal cardinal.has_pow
...
example : ℕ ≠ ℝ :=
begin 
  have nat_card : #ℕ = ℵ₀ := cardinal.mk_nat,
  have real_card : #ℝ = 2 ^ ℵ₀ := cardinal.mk_real,
  intro h,
  cases' h,
  rw nat_card at real_card,
  have card_lt : ℵ₀ < 2 ^ ℵ₀ := cardinal.cantor ℵ₀,
