import ..lectures.love08_operational_semantics_demo
...
infixr (name := seq_exercise) ` ;; ` : 90 := stmt.seq
...
infix (name := big_step_exercise) ` ⟹ ` : 110 := big_step
...
infix (name := equiv_exercise) ` ≈ ` := big_step_equiv
